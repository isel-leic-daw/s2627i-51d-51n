...
This document contains an introduction to React's effect management, namely the `useEffect` _hook_ function.
...
A way to accomplish this task is to use the link:https://developer.mozilla.org/en-US/docs/Web/API/Window/setInterval[`setInterval`] function:

* The component will have a `number` _state item_ holding the count value, used to present the current value in the UI, and defined using the `useState` _hook_ function.
...
* If the first operand is _falsy_, then the second operand to the `&&` is not evaluated and the overall expression evaluates to the value of the *first* operand, which in this case is `false` and is accepted (and ignored) as a `createElement` child.
* If the first operand is _truthy_, then the overall expression evaluates to the value of the second operand, that is `<Timer label="Second timer." />`.

CAUTION: Since the result is the value of the first operand, a _falsy_ operand that is not a boolean may end up being rendered.
For instance, `{items.length && <List/>}` shows a `0` when the array is empty, because `0` is _falsy_ but, unlike `false`, is *not* ignored as a child.
...
The following code defines a first attempt at the `Timer` implementation, using the `useEffect` _hook_ function.
[source, typescript]
----
...
IMPORTANT: the `useEffect` function and _effect function_ (also called the _effect callback_) are two distinct things. In the previous code, the _effect function_ is the lambda expression passed as the first argument to `useEffect`.

IMPORTANT: the `useEffect` function will be called on _every_ render of the component. The _effect function_ will be called _once_ for each placement of the component in the render-tree (when using an empty dependency array).
...
To do so, it is important to remember that `observedCount` is both _constant_ and _local_ to the `Timer` function.
...
Going back to the source code, we see that the callback function was defined as a lambda expression inside the _effect function_ lambda expression passed as argument to the `useEffect` in the _first_ render.
[source, typescript]
----
  useEffect(() => {
    console.log("Effect:", { ...props, observedCount });
...
----
This means that the callback function captures the _constant_ `observedCount` observed during the first render and uses it on _every_ periodic call.
...
A possible answer is to use an overload of the _state setter_ function that receives an _update function_ instead of the desired new value.
[source, typescript]
----
...
This means that there is no _resource leakage_: a timer that was removed from the render-tree closes all external resources (i.e. the interval). 
...
NOTE: This comparison is _shallow_: a dependency holding an object, an array, or a function that is recreated on every render is always considered different from the one in the previous call, resulting in the effect being cancelled and executed again on every render.
...
* The `useEffect` _hook_ function is the way to associate such side effects with a component. It receives an _effect function_ as the first argument and an _effect dependency array_ as the second one.
* `useEffect` and the _effect function_ are distinct things: `useEffect` is called on _every_ render, however the _effect function_ it receives is not. React decides when to call it, based on the dependency array.
...
* Every _props_ or _state_ observation used by the _effect function_ should be present in the dependency array. Otherwise the effect keeps using the values observed when it last ran, which is why the `Timer` ignored the `period` changes until `props.period` was added as a dependency.
* Each _state_ or _props_ observation is a _constant_ that is _local_ to one execution of the component function. A lambda expression defined during a render captures the observations of _that_ render and keeps using them whenever it is called in the future, even after new renders have occurred.
...
* When the new state depends on the current state, use the _state setter_ form that receives an _update function_ (e.g. `setCount(currState => currState + 1)`), instead of computing the new value from a captured observation.
...
* From the official link:https://react.dev[React documentation]:
  ** link:https://react.dev/learn/synchronizing-with-effects[Synchronizing with Effects].
  ** link:https://react.dev/learn/you-might-not-need-an-effect[You Might Not Need an Effect].
  ** link:https://react.dev/learn/lifecycle-of-reactive-effects[Lifecycle of Reactive Effects].
  ** link:https://react.dev/learn/separating-events-from-effects[Separating Events from Effects].
  ** link:https://react.dev/learn/removing-effect-dependencies[Removing Effect Dependencies].