...
This error will happen during runtime and not during development time, which would be much preferable.
In addition, the IDE lacks information to guide the programmer while they are using or defining the `someFunction` function.
...
The simple addition of `: string` to the function's parameter name is enough for the `someFunction(42)` error
to be detected during development.
The type system is also capable of inferring that the return type of `someFunction` and therefore determining that 
`const a: number = someFunction("hello")` is not a valid statement.
...
A bit strangely, tuples are defined using fixed-length arrays, since the JavaScript language doesn't have support for tuples during runtime.
...
Function types are first class citizens in the type world. Note how the arrow is `=>` and not `->` (as in Kotlin).
...
Until now, the TypeScript type system seems rather straightforward, at least for someone familiar with the Kotlin type system.
Union types are probably the first surprising thing to appear. 
...
In contrast with the Kotlin type system, the TypeScript type system is _structural_ and not _nominal_.
The name of a type is not relevant; only the type structure is relevant.
For instance, if `a` is a variable of type `A` and `b` is a variable of a different type `B`, then `b` can be assigned to `a`
if the structure of `B` is compatible with the structure of `A`, even if there isn't any sub-type relationship between `A` and `B`.
...
Type narrowing is the refinement of the type information associated with an expression, based on flow control analysis.
In Kotlin, a similar feature is called "smart casts".
In the example below, the type of `value` is different in the _then_ and _else_ branches of the `if` statement:

* Outside the `if` statement, the type of `value` is `number | string`.
* Inside the _then_ it is the more specific (_narrow_) `string` type.
* Inside the _else_ it is the more specific `number` type.
...
A value can be used to define a type that only allows that value.
For instance `"red"` can be used to define the type whose only value is the `"red"` string.
An example follows.
...
To distinguish between them, each object type defines a discriminator property (e.g. `kind`, `tag`) with a distinct type.
...
Every branch is an object type with the same `kind` property with a different literal type.
This discriminator property can then be used to do type-narrowing.
...
== Optional properties of object types

Object properties can be declared optional by using the `?` suffix on the _property name_.
...
== Readonly properties of object types

Object properties can be declared readonly by using the `readonly` prefix on the _property name_.

[#readonly-properties, typescript]
----
type Student = {
  name: string,
...
Object types can be defined with an open set of property names.
...
This type means that an object of type `Properties` can be indexed using a string and the result with be either a `string` or a `number` (or `undefined``).

Some usage examples:

[#index-signatures-examples, typescript]
----
type Properties = {
  [index: string]: string | number
}
...
const props: Properties = {
  name: "alice",
  id: 1234,
  // enrolled: true - ERROR Type 'boolean' is not assignable to type 'string | number'
}

function someFunctionUsingProperties(props: Properties) {
  // const name: string = props.name - ERROR Type 'string | number' is not assignable to type 'string'

  // This showns an unsoundness of the TypeScript type system
  // The statement below can fail at runtime, even if the program "type-checks"
  const foo: string | number = props.foo
  foo.toString() // This may fail in runtime.

  // Going through all the properties
  for(const [key, value] of Object.entries(props)) {
    const theKey: string = key
    const theValue: string | number = value
  }
}
----
...
* Change the type of the parameters so that the `any` type is _never_ used, and:
...
* Use the link:https://www.typescriptlang.org/play[TypeScript Playground] to test the type-checking.
...
Try not to use any DOM functions directly.
