...
Note how an application can use the same `Counter` component multiple times in different _render-tree positions_, with the expectation that they will operate independently.
An increment on the usage labelled `First counter` must not change what is shown on the usage labelled `Second counter`.

TIP: Try out the previous example and observe how `Counter` components in different positions work independently. Observe the browser's console and note
what happens when the application is loaded and what happens when each button is clicked.

NOTE: The _render-tree position_ terminology is taken from link:https://react.dev/learn/preserving-and-resetting-state[the React official documentation].
...
* That can be shared between renders of a component in the _same render-tree position_.
* That is independent between two distinct _render-tree positions_ of the same component.
* Whose change triggers a re-render of the component on the affected _render-tree position_.
...
The `useState` call then returns the pair `[0, setter_0]`, where `setter_0` is a function to set the value of the first state slot in the _first_ render position.
...
----
This nested `useState` call returns `[0, setter_1]`, where `setter_1` is a function to set the value of the first state slot in the _second_ render position.
...
The constant `observedCount` is _not_ the mutable counter state.
...
Note how the `Counter2` component now uses two independent state _items_:
...
Since each `useState` call is matched to a state slot by its _call order_, the sequence of `useState` calls must be exactly the same on every render of a component.
...
NOTE: This requirement is part of the link:https://react.dev/reference/rules/rules-of-hooks[Rules of Hooks] in the official React documentation.
...
To complement this lecture note, we recommend the reading of the link:https://react.dev/learn/preserving-and-resetting-state[Preserving and Resetting State] page in the official React documentation.
...
NOTE: this exercise suggests the use of the `useReducer` hook. See link:https://react.dev/learn/extracting-state-logic-into-a-reducer[Extracting State Logic into a Reducer] for more information about this state management hook.
